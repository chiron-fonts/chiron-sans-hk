v2.035 (2021/XX/XX)
====
- 上游版本更新至思源黑體 2.003 版。本版思源黑體由於新增可變式字型 (variable font) 格式支援，製作流程有所更改，所以中間字重 (Light, Normal, Regular, Medium, Bold) 的字圖大小跟前版略有分別。
- 新增可變式字型格式，字型名稱是 Chiron Sans HK VF。留意上游版本的可變式字型有幾個已知問題，因此請將這個格式看成先行/預覽版本。
